...
//-----------------------------------------------------------------------------
//  2025-09-01 - Version 1.9 - Fix to return camera to exact center screen.
//  2023-12-13 - Version 1.8 - Fixed bug with wait for camera
...
//-----------------------------------------------------------------------------
/*:
 * @plugindesc (v.1.9) Allows greater control over where the game camera is focused. View documentation for plugin commands.
 ...
 * ----------------------------------------------------------------------------
 * 
 *   Galv.CC.camOffset(x,y);      // set the offset x and y in pixels. This
 *                                // offsets that distance from the cam target
 *
 * ----------------------------------------------------------------------------
 *   PLUGIN COMMANDS
 * ----------------------------------------------------------------------------
 ...
 *
 * NOTES: 
 * - Make sure your screen resolution width/height are even numbers
...
Galv.CC.camOffset = function(x,y) {
    $gameMap._camOffsetX = x;
    $gameMap._camOffsetY = y;
};
...
Game_Map.prototype.updateScroll = function() {
    ...
    const screenX = (this.camTarget.screenX() + this._camOffsetX) * this.zoom.x;
    const screenY = (this.camTarget.screenY() + this._camOffsetY) * this.zoom.y;
    ...
    if (sx < 0.01) {
        let mx = Math.abs(screenX - Graphics.width / 2);
        sx = Math.min(mx,0.01);
    }
	if (sy < 0.01) {
        let my = Math.abs(screenY - Graphics.height / 2);
        sy = Math.min(my,0.01);
    }

	let sxsy = sx + sy;
	let hasMoved = sxsy != 0 && sxsy != this._camLastXY;
	this.camIsMoving = $gameMap.waitForCam ? hasMoved : false;
	this._camLastXY = sx+sy;
